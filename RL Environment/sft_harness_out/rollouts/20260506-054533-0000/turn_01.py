...
def flash_attention_forward_kernel(
    ...
    # Load Q block
    q_ptrs = Q + q_offset + tl.arange(0, BLOCK_M)[:, None] * stride_qs + tl.arange(0, BLOCK_D)[None, :] * 1
    q_mask = (block_m * BLOCK_M + tl.arange(0, BLOCK_M)[:, None]) < N_CTX
    q = tl.load(q_ptrs, mask=q_mask, other=0.0).to(tl.float32)
    
    # Iterate over K,V blocks
    for block_n in range(tl.cdiv(N_CTX, BLOCK_N)):
        k_ptrs = K + k_offset + block_n * BLOCK_N * stride_ks + tl.arange(0, BLOCK_N)[None, :] * stride_ks + tl.arange(0, BLOCK_D)[:, None] * 1
        v_ptrs = V + v_offset + block_n * BLOCK_N * stride_vs + tl.arange(0, BLOCK_N)[None, :] * stride_vs + tl.arange(0, BLOCK_D)[:, None] * 1
    ...
    # Store outputs
    o_ptrs = Out + o_offset + tl.arange(0, BLOCK_M)[:, None] * stride_os + tl.arange(0, BLOCK_D)[None, :] * 1
...
def flash_attention_backward_kernel(
    ...
    # Load DO
    do_ptrs = DO + do_offset + tl.arange(0, BLOCK_M)[:, None] * stride_dos + tl.arange(0, BLOCK_D)[None, :] * 1
    ...
    for block_n in range(tl.cdiv(N_CTX, BLOCK_N)):
        ...
        dv_ptrs = DV + batch_idx * stride_dvb + head_idx * stride_dvh + block_n * BLOCK_N * stride_dvs + tl.arange(0, BLOCK_N)[:, None] * stride_dvs + tl.arange(0, BLOCK_D)[None, :] * 1
        ...
        dk_ptrs = DK + batch_idx * stride_dkb + head_idx * stride_dkh + block_n * BLOCK_N * stride_dks + tl.arange(0, BLOCK_N)[:, None] * stride_dks + tl.arange(0, BLOCK_D)[None, :] * 1
    ...
    dq = dq * (1.0 / math.sqrt(BLOCK_D))
    
    # Store dQ
    dq_ptrs = DQ + dq_offset + tl.arange(0, BLOCK_M)[:, None] * stride_dqs + tl.arange(0, BLOCK_D)[None, :] * 1
