...
def flash_attention_forward_kernel(
    ...
    for block_n in range(tl.cdiv(N_CTX, BLOCK_N)):
        ...
        # k_t: [BLOCK_D, BLOCK_N], result: [BLOCK_M, BLOCK_N]
        k_t = tl.trans(k)
        s = tl.dot(q, k_t)
        s = s * scale
        ...
        # p: [BLOCK_M, BLOCK_N], v: [BLOCK_N, BLOCK_D] -> [BLOCK_M, BLOCK_D]
...
def flash_attention_backward_kernel(
    ...
    for block_n in range(tl.cdiv(N_CTX, BLOCK_N)):
        ...
        # p_t: [BLOCK_N, BLOCK_M], result: [BLOCK_N, BLOCK_D]
        p_t = tl.trans(p)
        dv_block = tl.dot(p_t, do)
        ...
        # do: [BLOCK_M, BLOCK_D], v: [BLOCK_N, BLOCK_D]
        # v_t: [BLOCK_D, BLOCK_N], result: [BLOCK_M, BLOCK_N]
        v_t = tl.trans(v)
        ds = tl.dot(do, v_t)
        ...
        # p_t: [BLOCK_N, BLOCK_M], dp: [BLOCK_M, BLOCK_N], result: [BLOCK_N, BLOCK_D]
        dk_block = tl.dot(p_t, dp)
        ...
        # Compute dQ: dQ += dP @ K
        # dp: [BLOCK_M, BLOCK_N], k: [BLOCK_N, BLOCK_D], result: [BLOCK_M, BLOCK_D]
        dq = dq + tl.dot(dp, k)
