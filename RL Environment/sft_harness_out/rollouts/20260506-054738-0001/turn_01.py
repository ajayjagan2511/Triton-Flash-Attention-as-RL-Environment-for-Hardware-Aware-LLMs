...
def flash_attention_forward_kernel(
    ...
    for block_n_id in range(0, num_blocks_n):
        ...
        s_block = s_block * (1.0 / math.sqrt(head_dim))
        
        # Mask out invalid positions (causal mask: only attend to past)
    ...
    o_block = o_block.to(Q.dtype.element_type)
...
def flash_attention_backward_kernel(
    ...
    for block_n_id in range(0, num_blocks_n):
        ...
        s_block = tl.dot(q_block, tl.trans(k_block)) * (1.0 / math.sqrt(head_dim))
        ...
        tl.store(dv_ptrs, dv_block, mask=offs_n[:, None] < seq_len)
        ...
        ds_block = ds_block * (1.0 / math.sqrt(head_dim))
        ...
        tl.store(dk_ptrs, dk_block, mask=offs_n[:, None] < seq_len)
    ...
    dq_block = dq_block.to(Q.dtype.element_type)
