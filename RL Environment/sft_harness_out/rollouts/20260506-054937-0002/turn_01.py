...
def flash_attention_forward_kernel(
    ...
    seq_len, head_dim,
    Bc, Br, BLOCK_SIZE_D: tl.constexpr,
):
    """
    ...
    tl.store(out_ptrs, out.to(Out.dtype.element_type), mask=out_mask)
...
def flash_attention_backward_kernel(
    ...
    for block_n in range(0, (seq_len + Bc - 1) // Bc):
        ...
        tl.atomic_add(dv_ptrs, dv_block.to(dV.dtype.element_type), mask=v_mask)
        ...
        tl.atomic_add(dk_ptrs, dk_block.to(dK.dtype.element_type), mask=k_mask)
